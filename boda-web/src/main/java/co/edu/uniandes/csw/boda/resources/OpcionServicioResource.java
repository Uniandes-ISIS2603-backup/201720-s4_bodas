...
public class OpcionServicioResource {
    ...
    @GET
    @Path("{id: \\d+}")
    public OpcionServicioDetailDTO getOpcionServicio(@PathParam("idProveedor") Long idProveedor ,@PathParam("id") Long id) throws BusinessLogicException {
       OpcionServicioEntity entity = opcionServicioLogic.findOpcionServicioById(id);
        if(entity==null)
       {
           throw new  WebApplicationException("El recurso /proveedor/" + idProveedor + "/opcionservicio/" + id + " no existe.", 404);
       }
  ...
  @GET
    ...
    public OpcionServicioDetailDTO updateOpcionServicio(@PathParam("idProveedor") Long idProveedor, @PathParam("id") Long id, OpcionServicioDetailDTO opcion) throws BusinessLogicException {
        ...
        opcion.setId(id);
        OpcionServicioEntity entity = opcionServicioLogic.getOpcionServicio(id);
        if(entity != null){
            throw new WebApplicationException("El recurso /proveedor/" + idProveedor + "/opcionservicio/" + id + " no existe.", 404);
        }
        return new OpcionServicioDetailDTO(opcionServicioLogic.updateOpcionServicio(idProveedor, entity));
    }
